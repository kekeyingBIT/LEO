function [Hd_mtx, Hf_mtx, Hd, Hf, ch_para] = TSL2(leo,user,ch,fq,act_set)
...
cellCenter = [0,0];
theta = linspace(0,2*pi,leo.N_p+1)+pi/2;
...
%         ( p(2)< sqrt(3)*p(1)+hexaPointY(1) );  % triangle board constraint

    p = 2*radius*rand(1,2)-radius;
    flagIn = (p(1)^2 + p(2)^2) < radius^2; % circle constraint
...
RxPosition(1,:) = hexaPointX(1:leo.N_p);
RxPosition(2,:) = hexaPointY(1:leo.N_p);
RxPosition(3,:) = repmat(leo.Height,[1,leo.N_p]);

% visiualize UT and LEO distribution
% figure, grid on
% plot3(hexaPointX,hexaPointY,zeros(1,leo.N_p+1),'b-', 'LineWidth',1.2); hold on
...
% zeta = 0:pi/180:2*pi; plot(radius*cos(zeta),radius*sin(zeta),'m-','LineWidth',1.2); hold on;
%% generate channel parameters
PHIs = zeros(leo.N_p,N_a);
THETAs = zeros(leo.N_p,N_a);
Ds = zeros(leo.N_p,N_a);
for np = 1:leo.N_p
% steering vector

phisp = 180/pi*atan( abs((TxPosition(3,:)-RxPosition(3,np))) ./ sqrt( (TxPosition(1,:)-RxPosition(1,np)).^2 + (TxPosition(2,:)-RxPosition(2,np)).^2 ) );
thetasp = 180/pi*atan2( (TxPosition(2,:)-RxPosition(2,np)), (TxPosition(1,:)-RxPosition(1,np)) );
dsp = sqrt( (TxPosition(1,:)-RxPosition(1,np)).^2 + (TxPosition(2,:)-RxPosition(2,np)).^2 + (TxPosition(3,:)-RxPosition(3,np)).^2 );
