function [data_recv_info_bit,data_recv_info_bit_Uncoded] = DD3(rec_sig,PilotIBI,...
    chMtx1,est_idx,mask_pos_all,dd_sel,para,Coded_Sel,...
    Coded_Info_bits,Uncoded_Info_bits,sframe_symbol,Qbits)
%% centralized processing for dd
% detection mode selection 
Quant_Sel = dd_sel;
% Quant_Sel = 'Centralized_Quant_VI';
...
data_recv_info = zeros(N_info_pblock_bit,N_block,N_a);
switch Coded_Sel
    case 'LDPC'
...
for blk =  1:N_block
    ...
    cir_data_sym = zeros(N_b,N_r,N_p);
    for np =  1:N_p
        ...
        cir_data_sym(:,:,np) = data_sym  + cir_data_blk- bf_blk- af_blk;
    end
    ...
    for sblk = 1 : N_subblock
        ...
        % reshape channel and observation

        Ht_all_fre = reshape(permute(chMtx1,[1,4,2,3]),[N_r*N_p,N_a,N_k]);
        cir_data_sym_reshape = reshape(cir_data_sym,[N_k,N_r*N_p]);
        data_subblock = cir_data_sym_reshape((sblk-1)*N_k+1:sblk*N_k,:);
        fre_data_sym = Fo_FFT'*data_subblock;  % data in sblk-th subblock
        
        % quantized feed back
        [y_rec,y_low,y_up,agc] = Quant_v3(fre_data_sym,Qbits);

        % quantization detection
        switch Quant_Sel
            case 'Centralized_Unquant_LS'
                for k = 1:N_k
                    y_s = fre_data_sym(k,:).';
                    ...
                    % LMMSE
                    x_s_u_fre = (channel_s_equ'*channel_s_equ + mean(Nvar)*mask.cr*eye(size(channel_s_equ,2)))\channel_s_equ'*y_s;
            ...
            case 'Centralized_Quant_VI_1'
                 x_nc_subblock = QVBSSD_v6(y_rec,y_low,y_up,Ht_all_fre,Nvar,M_ord,sframe_symbol(:,:,est_set),fre_data_sym,Fo_DFT);
                        
            case 'Centralized_Quant_VI_2'  % fully VI, failed about module 0 (Gaussian prior)
                x_nc_subblock = QVBSSD_v8(y_rec,y_low,y_up,Ht_all_fre,Nvar,M_ord,sframe_symbol(:,:,est_set),fre_data_sym,Fo_DFT);
            
            case 'Centralized_Quant_LS'
                for k = 1:N_k
            ...
            case 'Centralized_Quant_LS_mix'
                ...
                for k = 1:N_k
                    ...
                    y_s = zeros(N_r*N_p,1);
                    y_s(idx_unquant,:) = fre_data_sym(k,idx_unquant).';
                    y_s(idx_quant,:) = y_rec(k,idx_quant).';
        ...
        for u = 1:N_a
            ...
            data_recv_info((sblk-1)*N_info*N_bits+1:sblk*N_info*N_bits,blk,u) = data_u_llr;
...
% channel decoding
data_recv_info_all = reshape(data_recv_info,[N_info_pblock_bit*N_block,N_a]);

for u = 1:N_a
   
    u_idx = est_set(u);
    data_u_p_llr = data_recv_info_all(:,u);
    switch Coded_Sel
        case 'LDPC'
    ...
    data_recv_info_bit_Uncoded(:,u_idx) = data_u_p_llr'<0;  % coded data
