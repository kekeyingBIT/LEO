function [X_est_mu, lambda,N_iteration] = gmmv_oamp_app2(R,Phi,damp,niter,nvar,X,...
    U_all,S_all,V_all)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input:
% Y: Measurements
...
%         tau2 = v_ext;
        ...
        %  method 2 (31)
        S0_tilde = diag(1./(v2*diag(S0(:,1:M)).^2+sigma2));
        W_hat = V0*S0'*S0_tilde*U0';
        
        %  W_hat = v2*A' / ( v2*(A*A')+sigma2*eye(M) );
        tr1 = sum(diag((S0(:,1:M))'.*S0_tilde.*S0(:,1:M)));
        W_t = N/tr1*W_hat;  % Decorrelated
        ...
        beta = N/tr1;
        MtxTmp =eye(N)-beta*(S0'*S0_tilde*S0);
        tr2 = sum(diag(MtxTmp).^2);
        tr3 = beta^2* sum((diag(S0_tilde).^2).*(diag(S0(:,1:M)).^2));
        tau2 = 1/N*tr2*v2 + 1/N*tr3*sigma2;
